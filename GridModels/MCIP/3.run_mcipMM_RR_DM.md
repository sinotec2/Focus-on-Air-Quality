...
- `wrfout`之連結引用
   - 此處沒有使用`wrfout`的全名，而是在`bash`腳本中執行連結(see [ln_YYMM.cs](https://sinotec2.github.io/Focus-on-Air-Quality/GridModels/MCIP/2.ln_YYMM/#ln_yymmcs腳本))，因為全月的**WRF**模擬主要是以`bash`腳本控制，有較多的範本可以引用。
