...
- 工作流程：連結網頁、截圖、解讀、填入空格、下載檔案、重新命名。
- 如何有效的截圖：因[網站][epq]驗證碼是當使用者連結時隨機產生，如果先得到驗證碼圖檔後，再連結[網站][epq]，此時已經錯過原來的網頁，因此必須使用selenium啟動webdrive來控制整個程序。這部分的嘗試錯誤歷程詳見[驗證碼圖檔之取得]
