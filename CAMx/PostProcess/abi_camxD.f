...
c-----▒p▒▒mb
      ...
      do ii=1,ista
        ...
        do id=1,idays
          ...
          r1=maxval(obs((id-1)*24+1:i2,2,ii))
          if(r1.gt.0.)then
            ii1=ii1+1
            ii2=ii2+1
            r2=maxval(sim((id-1)*24+1:i2,2,ii))
            mb1(ii)=mb1(ii)+(r2-r1)/r1
            mb2=mb2+(r2-r1)/r1
          endif
        enddo
        mb1(ii)=mb1(ii)/real(ii1)
      enddo
      mb2=mb2/real(ii2)
c-----▒p▒▒ob,ge
      do isp=1,6
        ...
        if(.not.WithData(ii).or.sta_name(ii).eq.'*')cycle
          ...
          do ih=1,hh
            ...
            i2=i2+1
            r2=sim(ih,isp,ii)
            ob1(isp,ii)=ob1(isp,ii)+(r2-r1)*2/(r1+r2)
            ob2(isp)=ob2(isp)+(r2-r1)*2/(r1+r2)
            ge1(isp,ii)=ge1(isp,ii)+abs(r2-r1)*2/(r1+r2)
            ge2(isp)=ge2(isp)+abs(r2-r1)*2/(r1+r2)
          enddo
          ob1(isp,ii)=ob1(isp,ii)/real(i1)
          ge1(isp,ii)=ge1(isp,ii)/real(i1)
        enddo
        ob2(isp)=ob2(isp)/real(i2)
        ge2(isp)=ge2(isp)/real(i2)
      enddo
...
c-----▒g▒X
     ...
     +//' GE_NO2 OB_HC  GE_HC   OB_SO2 GE_SO2'
      data std/0.65,0.85,  0.15,0.35,  0.35,0.55, 0.35,0.55,0.65,0.85, 0.65,0.85,
     + 0.10, -1.0/!S       O           P          P         NV
