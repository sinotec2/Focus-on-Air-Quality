...
from ptse_sub import CORRECT, add_PMS, check_nan, check_landsea, FillNan
...
for c in V[1]:
  ...
  if c in ['CO', 'CP_NO']: continue
  ic=lspec.index(c)
  nc.variables[c][:,:nopts]=np.array(df[c]).reshape(ntm,nopts)
  print(c)
#CO are store temperly in NO to speed up the process
c='CO'
if c in df.columns:
  CO=np.array(df[c]).reshape(ntm,nopts)
  nc.variables['NO'][:]=CO[:]
  nc.variables['CO']=nc.variables['NO']
