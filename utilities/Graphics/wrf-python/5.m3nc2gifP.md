...
濃度範圍|單一檔不同時間，在同一個python內即可決定|另外讀取決定，將極值輸入每一個小時之作業，才能做出相同濃度範圍的圖形|需撰寫[新的程式][mxnNC]
...
- 雖然不會影響`ncrcat`的執行，此處還是將其改成2碼整數(00~23)
  - `ncrcat`使用`$(echo ${fnames[@]})`方式來連接所有小時的片段檔案，沒有改變。這個做法的好處是：
    1. 個數不定、保持彈性
...
- 因[循序版][4.]程式的濃度範圍，並非真正的極大與極小值，而是一定累積頻率的範圍。沒有現成的程式（如mxNC,mnNC等等）可用，需要另外寫一個。([mxnNC][mxnNC])
...
- png檔案的串連：與前述ncrcat一樣，採用`echo ${pngs[@]}`方式。好處同[上](#每個nc檔之命名方式)
...
[mxnNC]: <ttps://github.com/sinotec2/Focus-on-Air-Quality/blob/main/utilities/Graphics/wrf-python/mxnNC> "[mxnNC]"
