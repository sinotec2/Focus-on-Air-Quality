...
for i in range(ny*nx):
  ...
  boo=(dist<=(nc.XCELL*3.0)**2) & (dist>0)
  idx=np.where(boo)[0]
...
for day in range(5):
  ...
  for t in range(0,nt,3):
    ...
    for i in range(nozn):
      ...
      ozn[i]['header']["parameterNumberName"]="Ozone_Mixing_Ratio"
      var=o3[t,:,:]*1000
...
o38[7:,:,:]=var[:,:,:]
for i in range(nozn):
  ozn[i]['header']["parameterNumberName"]="Ozone_Mixing_Ratio_8hr"
for day in range(5):
  fname=fcst.replace('YYYYMMDD',YMDs[day].replace('-',''))
  if not os.path.isfile(fname):continue
  nc = netCDF4.Dataset(fname, 'r')
  ...
  for t in range(0,nt,3):
    ...
    pwd='/nas1/Data/javascripts/D3js/earthFcst'+grds+'/public/data/weather/current/'
    hh=bdate.strftime("%H00")
    var=o38[day*24+t,:,:]*1000
    if np.sum(var)==0.:continue
    for i in range(nozn):
      ozn[i]['data']=list(np.flip(np.where(var!=var,0,var),axis=0).flatten())
