...
- slurm的啟動
  ...
  - `$LMOD_CMD`為內定的lmod程式
  - 詳參[csh 中執行module](../TWNEPA_RecommCMAQ/module_slurm.md#csh-中執行module)
...
- 這支程式可以順利執行，但不接受pnetcdf寫出的檔案。需進行`$BIN/trans_wrfout.py`轉檔(詳見[因應intel MPI轉換wrfout格式](../../wind_models/WRFOUT/2.TransWrfout.md))。
