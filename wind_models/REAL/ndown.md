...
- 執行雙向巢狀網格的[real.exe][real]、上層單層的[wrf.exe](https://sinotec2.github.io/Focus-on-Air-Quality/wind_models/REAL/dowrf/)之後。將namelist.input進行備份、修改。
- &time_control下
...
- &domains
  ...
  - e_we、e_sn、dx、dy、(不動)保持母、子網格的設定與執行[real.exe][real])時一樣
  - time_step = 240  → 80 。時間步階適度調整(保持dt/dx<6)，以方便子網格wrf之執行。

### 當[real.exe][real]同時run了超過2層
...
- 準備wrfndi_d02：Rename(`mv`) the wrfinput_d02 file to wrfndi_d02
  - wrfinput_d02必須是執行[real.exe][real]所產生下一層子網格的初始條件。(不能是單層[real][real]的wrfinput_d01)
...
[real]: <https://sinotec2.github.io/Focus-on-Air-Quality/wind_models/REAL/doreal_4Nests.sh/> "doreal_4Nests.sh"