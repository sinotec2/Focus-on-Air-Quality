...
- png合併成gif需給定背景與尺寸的理由詳見[格式轉換](https://sinotec2.github.io/Focus-on-Air-Quality/utilities/Graphics/imageMagicks/#格式轉換)
...
```
### 浮動的濃度等級
- 如前所述，濃度等級需要幾個項目是浮動的(至少想到4個)：
  - 1.線性或對數色階，改成彈性判斷
    - 最大/最小比值高於15：非線性、對數色階，適用在煙流增量分布，
    - 低於15：線性色階，適用在濃度變化不大的空品項目。
  - 2.極值的設定：全時間、全域、**正值**之
    - `99.99 percentile` (等級之最大值)
    -  `0.01 percentile` (等級之最小值)
    - 不服務**負值**的理由是：
  ...
  - 4.有效位數(小數及大數)
    ...
    - `N=int(3-np.log10(mxv))`，大致上可以保持有3~4個有效數字，測試結果如下
...
|:--:|:--:|:--:|
| <b>公版[後製工具](https://sinotec2.github.io/Focus-on-Air-Quality/GridModels/TWNEPA_RecommCMAQ/post_process#table-of-contents-in-post_process)</b>|<b>[VERDI](https://sinotec2.github.io/Focus-on-Air-Quality/utilities/Graphics/VERDI/VERDI_Guide/)</b>|<b>m3nc2gif</b>|