...
- 為整合全球地面、高空、衛星等觀測成為系統性、網格化的數據，各大氣象中心持續投入所謂「再分析」工作，將觀測與模式整合成具系統性檔案。
- 再分析數據的時空解析度不會太高、但作為模式模擬的初始與邊界條件卻非常足夠。
- 因大多數再分析數據是以`grib`格式存檔，所以下載後要進行轉檔，以準備下一階段的切割與網格化([metgrid.exe]())。

#### 檔案解讀的工作核心：建立對照關係
- 由於各單位再分析檔案內的變數名稱、單位等等都不相同，解讀時就需要逐一進行對照、單位與名稱轉換、這需要一個完整的對照表，稱之為**Vtable**(**V**ariable dictionary **table**)。
- WPS累積至今已有有40種檔案、變數對照關係表，可以按照再分析數據的來源選用。包括：
```bash
